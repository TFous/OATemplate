...
import choseClient from './choseClient'
@Component({
    name: 'setMoney',
    mixins: [template],
    watch: {
...
export default class setMoney extends Vue {
    ...
    setMoneyAuth:boolean= false//浏览次席信息权限
    ...
    setAuth(auth){
        let _self = this
        if(auth.profile.role.indexOf('WPBS_WithdrawManage') !== -1){
            this.setMoneyAuth = true
        }else{
            this.setMoneyAuth = false
    ...
    data() {
        return {
            options: Object.assign({}, Vue.prototype.$xvuex.options, {
                ...
                title: '次席系统',  // 本页面名称
                gridKey: 'choseWithdrawClient',  // 本页面 Eng名，唯一
